...
void HighwayHashReset(const uint64_t key[4]) {
    HighwayHashState *state = &highway_hash_init_state;
...
uint64_t HighwayHash64(const uint8_t *data, size_t size) {
    memcpy(&target_state, &highway_hash_init_state, sizeof(HighwayHashState));
    ProcessAll(data, size, &target_state);
    return HighwayHashFinalize64(&target_state);
}

//int main(void) {
//    const uint64_t seed[4] = {1, 2, 3, 4};
//    HighwayHashReset(seed);
//    const uint8_t key[4] = {2, 3, 4, 6};
//    printf("%llu", HighwayHash64(key, 4));
//    return 0;
//}
